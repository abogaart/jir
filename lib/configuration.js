    ...
    Seq()
        ...
        .seq(function(globalSettingsFile) {
            ...
            if (!shell.which(settings.gitExec)) {
                tools.die('Jir requires git to be installed, could not detect it at %s', settings.gitExec);
            }

            if (!shell.which(settings.mvnExec)) {
                tools.die('Jir requires Maven3 to be installed, could not detect it at %s', settings.mvnExec);
            }
        ...
        .seq(function(dataDirExists) {
            ...
            if (!dataDirExists) {
                ...
                tools.info('Welcome to Jir, a Hippo tool for commandline geeks. Currently the dataset is being checked out and installed. This will only take a few seconds.\n');
